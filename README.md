...
- ✅ **脚注支持** - 支持 `[^1]` 语法，自动编号，点击跳转，完美适配微信公众号
...
支持 Markdown 脚注语法，适合添加引用说明、参考资料等：

```markdown
微信 Markdown 编辑器[^1] 是一款高度简洁的编辑器[^note]。
...
- 正文中的 `[^1]` 会渲染为上标样式的引用标记
- 脚注定义会自动收集并在文末渲染为编号列表
- 支持点击脚注编号跳转到对应定义，点击 ↩ 返回原文
- 支持数字编号 `[^1]` 和命名引用 `[^note]` 两种格式
