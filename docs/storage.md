...
Web 应用使用 IndexedDB（数据库名 `doocs-md`，当前版本 `4`）作为主存储，localStorage 作为降级方案。所有存储初始化逻辑位于 `apps/web/src/storage/`。
...
| v4   | 表情迁移到云端；删除本地 `emoji` store、旧包元数据和遗留 Blob             |
...
export async function upgradeDB(db, oldVersion, _newVersion, transaction) {
  ...
  if (oldVersion < 4) {
    // 清理旧表情元数据和二进制，再删除 emoji object store
  }
}
```
...
v4 起表情不再保存在浏览器 IndexedDB：

- 默认酷安包和登录用户的单套自定义表情元数据存储在 md-api 的 D1
- 图片二进制存储在专用私有 R2，通过 `/emojis/assets/:id` 提供稳定 URL
- `services/emoji/urlResolver.ts` 维护当前 manifest 的 `id → URL` 内存映射
- 导出时 `lib/export/inlineEmojiImages.ts` 拉取远端图片并转换为 base64，确保微信编辑器粘贴结果不依赖远端

升级到 v4 会按产品策略清理旧 `MD__emoji_packs` 和本地 Blob，不迁移旧表情。详细设计见 `apps/web/src/stores/emojiPack.ts` 和 `apps/api/src/emoji.ts`。
