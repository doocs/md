        ...
        <div v-if="!isShowForm">
          ...
          <section class="mb-6">
            ...
            <Accordion v-model="expandedValues" type="single" collapsible class="space-y-2">
              ...
              >
                <AccordionTrigger class="flex items-center gap-3 p-3 sm:p-4 hover:no-underline [&>svg]:size-3.5">
                  ...
                  <Button
                    variant="default"
                    size="sm"
                    class="h-7 px-2.5 text-xs gap-1 shrink-0"
                ...
                <AccordionContent class="px-3 sm:px-4 pb-3 pt-0 space-y-3 bg-muted/20">
                  <div v-if="def.props.length > 0">
                    ...
                    <div class="hidden sm:block rounded-lg border overflow-hidden">
                      <table class="w-full text-xs">
                        ...
                        <tbody>
                          <tr v-for="prop in def.props" :key="prop.name" class="border-t border-border/50">
                            <td class="px-3 py-2">
                              <code class="font-mono text-primary font-medium">{{ prop.name }}</code>
                            </td>
                            <td class="px-3 py-2">
                              <span class="inline-flex items-center px-1.5 py-0.5 rounded border text-[10px] font-medium" :class="propTypeBadge(prop).class">{{ propTypeBadge(prop).label }}</span>
                            </td>
                    ...
                    <div class="sm:hidden space-y-2">
                      <div v-for="prop in def.props" :key="prop.name" class="rounded-lg border bg-card p-3 space-y-1">
                  ...
                  <div>
                    ...
                    <div class="relative group">
                      ...
                      <Button variant="ghost" size="icon" class="absolute right-1.5 top-1.5 size-6 opacity-0 group-hover:opacity-100 transition-opacity" @click="copySnippet(def)">
                  ...
                  <div v-if="isMpProfile(def)" class="border-t pt-3 space-y-2">
                    ...
                    <div v-else class="space-y-1.5">
                      <div v-for="account in mpAccountsStore.accounts" :key="account.id" class="flex items-center gap-2 p-2 rounded-lg border bg-card hover:bg-muted/50 transition-colors">
                        <img v-if="account.logo" :src="account.logo" class="size-7 rounded-full object-cover shrink-0" :alt="account.name">
              ...
              >
                ...
                <AccordionContent class="px-3 sm:px-4 pb-3 pt-0 space-y-3 bg-muted/20">
                  ...
                  <div class="flex items-center gap-2 pt-1 border-t">
                    <Button variant="outline" size="sm" class="h-7 px-2.5 text-xs gap-1" @click="openEditForm(def)">
                      <Pencil class="size-3" />
                      编辑
                    </Button>
                    <Button variant="outline" size="sm" class="h-7 px-2.5 text-xs gap-1 text-red-500 hover:text-red-600 hover:border-red-300" @click="openDeleteConfirm(def)">
                      <Trash2 class="size-3" />
                      删除
                    </Button>
                  </div>
