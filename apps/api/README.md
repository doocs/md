...
- `GET  /upload/config` 查询默认图床 API 上传是否可用
- `POST /upload` 默认图床备用上传（GitHub 或 R2，由服务端配置）
...
```bash
pnpm api exec wrangler secret put UPLOAD_GITHUB_TOKENS   # 逗号分隔 PAT，支持 doocsmd 混淆
```
...
1. 创建 bucket 并在 `wrangler.toml` 取消注释 `[[r2_buckets]]`
2. 设置 `UPLOAD_BACKEND = "r2"` 与 `UPLOAD_R2_PUBLIC_URL`（R2 自定义域名公开前缀，如 `https://images.doocs.org`）

前端在 `apps/web/.env` 设置 `VITE_UPLOAD_VIA_API=true`（并与服务端 `UPLOAD_ENABLED` 同步开启）后，默认图床经 `POST /upload` 代理，密钥不经过浏览器。无需运行时调用 `/upload/config`。
